...
int main(int argc, char * argv[])
{
	...
	float *matrix_transformed = malloc(n * n * sizeof(float));
	...
	timing_t timer1;
	timer_start(&timer1);
	transform(n, mat, matrix_transformed, VECTOR_SIZE);
	MatVecMultiply(size, n, matrix_transformed, vec, output);
	timer_stop(&timer1);
	...
	if (trace == 1) {
		...
		for (int i=0; i<n; i++){
			for (int j=0; j<n; j++){
				printf("%.2f " , matrix_transformed[i*n+j]);
			}
			printf("\n");
		}
		printf("\nInput vector \n");
